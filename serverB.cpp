...
#define MYPORT "31054"   // the port users will be connecting to
...
void constructgraph(){//read the data2.txt file and build the connection map
	string line;
	ifstream infile;
	...
	if(!infile.is_open()){
		cout<<"The file data2.txt cannot be opened successfully"<<endl;
		exit(-1);
	}
...
int main(void)
{
	...
	while(1){
		...
		if(countrymap[country].find(stoi(id))==countrymap[country].end()){
			...
			strncpy(buf, reply.c_str(), reply.length() + 1);
